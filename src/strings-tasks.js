...
function sumOfCodes(str) {
  ...
  return [...str].reduce((acc, cur) => acc + cur.charCodeAt(0), 0);
}
...
function isPalindrome(str) {
  const cleadedStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  const reversedStr = cleadedStr.split('').reverse().join('');
  return cleadedStr === reversedStr;
}
